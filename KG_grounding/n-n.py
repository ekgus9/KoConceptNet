...
f = open(datapath+"type_constrain.txt", "w")
f.write("%d\n"%(len(rellef)))
for i in rellef:
...
f = open(datapath + "test2id.txt", "r")
f11 = open(datapath + "1-1.txt", "w")
f1n = open(datapath + "1-n.txt", "w")
fn1 = open(datapath + "n-1.txt", "w")
fnn = open(datapath + "n-n.txt", "w")
fall = open(datapath + "test2id_all.txt", "w")
tot = (int)(f.readline())
fall.write("%d\n"%(tot))
f11.write("%d\n"%(s11))
f1n.write("%d\n"%(s1n))
...
#######################
commongend = '../dataset/new_ko/kommongen_dev.src_alpha.txt'

for file_name in [commongend.replace("dev", 'train'), commongend, commongend.replace("dev", 'test')]: #
    with open(file_name, "r", encoding='utf-8') as ind_f:
        ind_f = list(ind_f)
        for i in range(len(ind_f)):
            with open(file_name.replace("src_alpha","index"), 'a', encoding='utf-8') as f:
                f.write(str(i) + '\n')