...
from transformers import AutoTokenizer, BartTokenizer, PreTrainedTokenizerFast, T5Tokenizer #DistilBertTokenizer
# from kobart import get_pytorch_kobart_model, get_kobart_tokenizer

# os.chdir("../")
sys.path.append('/home/mnt/dhaabb55/Concept/KG-BART/KGBART')
...
from KGBART_model.modeling_t5 import T5ForConditionalGeneration
...
def main():
    ...
    parser.add_argument("--bart_model", default="gogamza/kobart-base-v2", type=str,
                        help="Bert pre-trained model selected in the list: bert-base-uncased, "
                             "bert-large-uncased, bert-base-cased, bert-base-multilingual, bert-base-chinese.")
    parser.add_argument("--config_path", default=None, type=str,
                        help="Bert config file path.")
    parser.add_argument("--output_dir",
                        default="../output/train_kobart_t",
                        type=str,
                        help="The output directory where the model predictions and checkpoints will be written.")
    parser.add_argument("--log_dir",
                        default="../log/train_kobart",
                        type=str,
    ...
    if args.bart_model == 'gogamza/kobart-base-v2':
    # tokenizer = BartTokenizer.from_pretrained(
    #     args.bart_model, do_lower_case=args.do_lower_case)
        data_tokenizer = PreTrainedTokenizerFast.from_pretrained(args.bart_model)
        
    elif args.bart_model == 'google/mt5-base':
        data_tokenizer = T5Tokenizer.from_pretrained(args.bart_model)
    ...
    bi_uni_pipeline = [
        seq2seq_loader.Preprocess4Seq2seq(list(tokenizer.get_vocab().keys()), tokenizer.convert_tokens_to_ids,
    ...
    entity_id = os.path.join(
        args.data_dir, args.tgt_file if args.tgt_file else 'entity2id.txt')
    entity_embedding_path = os.path.join(
        args.data_dir, args.tgt_file if args.tgt_file else 'ent_embeddings')

    relation_id = os.path.join(
        args.data_dir, args.tgt_file if args.tgt_file else 'relation2id.txt')
    relation_embedding_path = os.path.join(
        args.data_dir, args.tgt_file if args.tgt_file else 'rel_embeddings')

    entity_embedding = np.array(pickle.load(open(entity_embedding_path, "rb")))
    entity_embedding = np.array(list(np.zeros((4, 768))) + list(entity_embedding)) #1024
    ...
    if args.do_train:
        ...
        fn_src = os.path.join(
            args.data_dir, args.src_file if args.src_file else 'kommongen_train.src_alpha.txt')
        fn_tgt = os.path.join(
            args.data_dir, args.tgt_file if args.tgt_file else 'kommongen_train.tgt.txt')
        fn_onehop = os.path.join(
            args.data_dir, args.tgt_file if args.tgt_file else 'kommongen_train.onehop_5.txt')
    ...
    if args.do_eval:
        ...
        fn_src = os.path.join(
            args.data_dir, args.src_file if args.src_file else 'kommongen_dev.src_alpha.txt')
        fn_tgt = os.path.join(
            args.data_dir, args.tgt_file if args.tgt_file else 'kommongen_dev.tgt.txt')
        fn_onehop = os.path.join(
            args.data_dir, args.tgt_file if args.tgt_file else 'kommongen_dev.onehop_5.txt')
    ...
    # args.bart_model = get_pytorch_kobart_model()
    
    if (recover_step is None) and (args.model_recover_path is None):
        # if _state_dict == {}, the parameters are randomly initialized
        # if _state_dict == None, the parameters are initialized with bert-init
        _state_dict = {} if args.from_scratch else None
        if 'bart' in args.bart_model :
        ...
        elif 't5' in args.bart_model:
            model = T5ForConditionalGeneration.from_pretrained(args.bart_model, entity_weight=entity_embedding,
                                                             relation_weight=relation_embedding)
        global_step = 0
    else:
        if recover_step:
        ...
        if 'bart' in args.bart_model:
            ...
            model = T5ForConditionalGeneration.from_pretrained(args.bart_model, state_dict=model_recover,entity_weight=entity_embedding,
                                                             relation_weight=relation_embedding)
    # if args.local_rank == 0:
    #     dist.barrier()
