        ...
        if pretraining_KG is None:
            with open(file_src, "r", encoding='utf-8') as f_src, open(file_tgt, "r", encoding='utf-8') as f_tgt, \
                ...
                for i, entityid in enumerate(entity_line):
                    if i == 0: continue
                    entity_id[''.join(entityid.split()[:-1])] = int(entityid.split()[-1]) + 4
                    id_entity[int(entityid.split()[-1]) + 4] = ''.join(entityid.split()[:-1])
                ...
                for i, relationid in enumerate(relation_line):
                    if i == 0: continue
                    relation_id[''.join(relationid.split()[:-1])] = int(relationid.split()[-1]) + 4
                    id_relation[int(relationid.split()[-1])] = ''.join(relationid.split()[:-1])
                ...
                for src, tgt, onehop in zip(f_src, f_tgt, concept_list):
                    ...
                    for wi, src1 in enumerate(src.split()):
                        ...
                        if src1 not in entity_id:
                            entity_id[src1] = 0 #len(entity_id)
                    ...
                    for key in onehop.keys():
                        ...
                        for word_rel in onehop[key].keys():
                            ...
                            # try : entity_id[word_rel]
                            # except:
                            #     entity_id[word_rel] = 0 #list(entity_id.values())[-1] + 1

                            # try : relation_id[onehop[key][word_rel][0]]
                            # except:
                            #     relation_id[onehop[key][word_rel][0]] = 0 #list(relation_id.values())[-1] + 1
                            #     # print(onehop)
                            #     # print(onehop.keys())
                            #     # print(onehop[key])
                            #     # print(onehop[key].keys())
                            #     # exit()

                            if len(each_concept_expand) >= 5: break
                            # print(word_rel)
                            if word_rel not in entity_id:
                                entity_id[word_rel] = 0 #len(entity_id)
...
class Preprocess4Pretrain(Pipeline):
    ...
    def __call__(self, instance):
        ...
        def truc(tet):
            if self.max_len < len(tet) : 
                tet = tet[:self.max_len]
                
            return tet

        input_ids = truc(input_ids)
        input_entity_id = truc(input_entity_id)
        subword_mask = truc(subword_mask)
        word_mask = truc(word_mask)
        word_subword = truc(word_subword)
        decoder_input_ids = truc(decoder_input_ids)
        labels = truc(labels)
        decoder_attention_mask = truc(decoder_attention_mask)
        # concept_entity_expand = truc(concept_entity_expand)
        # concept_relation_expand = truc(concept_relation_expand)
