...
from transformers import BartTokenizer, PreTrainedTokenizerFast
...
def main():
    ...
    parser.add_argument("--input_file", type=str,
                        default="../../dataset/ko/kommongen_test.src_alpha.txt ",
                        help="Input file")  # "../../../../dataset/final_data/commongen/commongen.dev.single_alpha.txt"
    parser.add_argument('--subset', type=int, default=0,
                        help="Decode a subset of the input dataset.")
    parser.add_argument("--output_dir", type=str, default="../output/KoBart/best_model/Gen",
                        help="output dir")  # "../../../../output/unilm/Gen/model_base.5.bin.test"
    parser.add_argument("--output_file", type=str, default="model.best",
    ...
    args.fp16 = False
    args.bart_model = 'gogamza/kobart-base-v2'
    ...
    tokenizer = PreTrainedTokenizerFast.from_pretrained(args.bart_model)
    ...
    bi_uni_pipeline.append(
        seq2seq_loader.Preprocess4Seq2seqDecoder(list(tokenizer.get_vocab().keys()), tokenizer.convert_tokens_to_ids,
    ...
    entity_id = os.path.join(
        args.data_dir, 'entity2id.txt')
    entity_embedding_path = os.path.join(
        args.data_dir,  'ent_embeddings')

    relation_id = os.path.join(
        args.data_dir, 'relation2id.txt')
    relation_embedding_path = os.path.join(
        args.data_dir, 'rel_embeddings')

    file_onehop = os.path.join(
        args.data_dir, 'kommongen_test.onehop_5.txt')
        ...
        for src, onehop in zip(fin.readlines(), concept_list):
            ...
            for key in onehop.keys():
                ...
                for word_rel in onehop[key].keys():
                    ...
                    try:
                        ng_entity_id = entity_id[word_rel.replace(' ','_')]
                    except:
                        ng_entity_id = 0
    ...
    with tqdm(total=total_batch) as pbar:
        while next_i < len(input_lines):
            ...
            with torch.no_grad():
                ...
                # print(traces)
                # print(tokenizer.batch_decode(traces))
                # exit()
                # if args.beam_size > 1:
