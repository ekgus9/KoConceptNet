...
# os.chdir("../")
sys.path.append('/home/mnt/dhaabb55/Concept/KG-BART/KG_grounding/')
...
def main():
    ...
    entity_embedding_path = os.path.join(
        args.data_dir, args.tgt_file if args.tgt_file else 'distillbert_tokenizer_entity')
    ...
    entity_embedding = DistilBertTokenizer.from_pretrained(entity_embedding_path)
    relation_embedding = DistilBertTokenizer.from_pretrained(relation_embedding_path)

    if args.do_train:
        print("Loading Train Dataset", args.data_dir)
        if args.pretraining_KG:
            file_oracle = os.path.join(args.data_dir, 'entity2id.txt')
        fn_src = os.path.join(
            args.data_dir, args.src_file if args.src_file else 'kommongen_train.src_new.txt')
        fn_tgt = os.path.join(
