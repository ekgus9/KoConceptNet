    ...
    def forward(
        ...
        if position_bias is None:
            if not self.has_relative_attention_bias:
                ...
                # print(real_seq_length, key_length)
                # print('!')
            else:
                # print(real_seq_length, key_length)
        ...
        # print('**')
        # print(scores.shape)
        # print(position_bias_masked.shape)
...
class GATSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if query.shape == entity.shape: 
            new_query = self.conv1D(query.transpose(1,2))[:,:,:bsz].transpose(1,2)
        else:
            new_query = self.conv1D(query.transpose(0,1).transpose(1,2))[:,:,:bsz].transpose(1,2) # self.conv1D(query.transpose(0,1).transpose(1,2))[:,:,:tgt_len].transpose(1,2)
        query = new_query
...
class EncoderGATLayer(nn.Module):
    def __init__(self, config: T5Config):
        ...
        self.self_gat_attn = GATSelfAttention(
            self.embed_dim, config.num_heads, dropout=config.dropout_rate,
        )
        self.dropout_rate = config.dropout_rate
        
        # self.normalize_before = config.normalize_before
        self.normalize_before = False
        self.self_attn_layer_norm = T5LayerNorm(self.embed_dim, eps=config.layer_norm_epsilon)
        self.activation_fn = ACT2FN[config.dense_act_fn]
        self.activation_dropout = config.dropout_rate
        self.fc1 = nn.Linear(self.embed_dim, config.d_ff)
        self.fc2 = nn.Linear(config.d_ff, self.embed_dim)
        self.final_layer_norm = T5LayerNorm(self.embed_dim, eps=config.layer_norm_epsilon)

    def forward(self, x, entity_input, encoder_padding_mask, word_mask, word_subword, output_attentions=False, position_bias = None):
        """
        Args:
...
class DecoderGATLayer(nn.Module):
    def __init__(self, config: T5Config):
        ...
        self.self_attn = T5LayerSelfAttention(
            embed_dim=self.embed_dim, num_heads=config.decoder_attention_heads, dropout=config.dropout_rate,
        )
        self.dropout = config.dropout_rate
        self.activation_fn = ACT2FN[config.dense_act_fn]
        self.activation_dropout = config.dropout_rate
        ...
        self.encoder_attn_layer_norm = T5LayerNorm(self.embed_dim, eps=config.layer_norm_epsilon)
        self.fc1 = nn.Linear(self.embed_dim, config.d_ff)
        self.fc2 = nn.Linear(config.d_ff, self.embed_dim)
        self.final_layer_norm = T5LayerNorm(self.embed_dim, eps=config.layer_norm_epsilon)
...
class T5Stack(T5PreTrainedModel):
    def __init__(self, config, embed_tokens=None, \
                embed_entitys = None, embed_relations = None):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                if self.is_decoder:
                    ...
                    if i < int(self.encoder_layers / 2):
                        ...
                        entity_embeds = F.dropout(entity_embeds, p=self.dropout_rate, training=self.training)
                        entity_embeds = entity_embeds.transpose(0, 1)
                        # print(layer_module)
                        layer_outputs = layer_module(hidden_states, entity_input=entity_embeds, encoder_padding_mask=attention_mask, word_mask=word_mask, word_subword=word_subword, output_attentions=output_attentions, position_bias=position_bias,)
...
class T5Model(T5PreTrainedModel):
    ...
    def __init__(self, config: T5Config, entity_weight, relation_weight):
        super().__init__(config)
        self.shared = nn.Embedding(config.vocab_size, config.d_model)
        
        self.shared_entity = nn.Embedding(entity_weight.shape[0], entity_weight.shape[1], padding_idx=config.pad_token_id)
        self.shared_entity.weight.requires_grad = False
        self.shared_relation = nn.Embedding(relation_weight.shape[0], relation_weight.shape[1], padding_idx=config.pad_token_id)
        ...
        self.encoder = T5Stack(encoder_config, self.shared, self.shared_entity, self.shared_relation)

        decoder_config = copy.deepcopy(config)
        decoder_config.is_decoder = True
        decoder_config.is_encoder_decoder = False
        decoder_config.num_layers = config.num_decoder_layers
        self.decoder = T5Stack(decoder_config, self.shared, self.shared_entity, self.shared_relation)
...
class T5EncoderModel(T5PreTrainedModel):
    ...
    def __init__(self, config: T5Config, embed_entitys, embed_relations):
        super().__init__(config,)
        self.shared = nn.Embedding(config.vocab_size, config.d_model)
        ...
        encoder_config.use_cache = False
        encoder_config.is_encoder_decoder = False
        self.encoder = T5Stack(encoder_config, self.shared, self.embed_entitys, self.embed_relations)
    ...
    def _prune_heads(self, heads_to_prune):
        ...
        """
        for layer, heads in heads_to_prune.items():
            self.encoder.block[layer].layer[0].T5LayerSelfAttention.prune_heads(heads)
